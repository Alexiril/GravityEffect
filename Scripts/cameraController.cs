...
public class cameraController : MonoBehaviour
{
    ...
    [SerializeField]
    private UnityEvent mainActionLeft = default, mainActionRight = default;
    ...
    {
        if (Mathf.Approximately(IA.FindAction("Person/MainActionLeft").ReadValue<float>(), 1))
        {
            mainActionLeft.Invoke();
        }
        if (Mathf.Approximately(IA.FindAction("Person/MainActionRight").ReadValue<float>(), 1))
        {
            mainActionRight.Invoke();
        }
    ...
    {
        Vector2 input = IA.FindAction("Person/ViewAround").ReadValue<Vector2>();
        var i = input.x;
        input.x = -input.y;
        input.y = i;
    ...
    private void UpdateGravityAlignment()
    {
        ...
        Vector3 toUp = unusualGravity.GetUpAxis(focusPoint);
